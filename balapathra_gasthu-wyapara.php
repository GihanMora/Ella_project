  ...
  <li><a href="balapathra_gasthu-welada.php">වෙළඳ බලපත්‍ර</a></li>
	...
	<?php
		...
		if(isset($_POST['enter'])){
			...
			$ck="select ankaya from balapathra_wyapara WHERE nama='$name'";
			$rr=mysqli_query($connection, $ck);
			if($w = mysqli_fetch_array($rr)){
				echo "Update entry";
				$insert="UPDATE balapathra_wyapara SET $mon='$payment' WHERE nama='$name'";
			}
			else{
				echo "new enty";
				$insert="INSERT INTO balapathra_wyapara(nama,address,balapathra_ankaya,ekathuwa,masaya,$mon) VALUES('$name','$addres',$num,'$payment','$month','$payment')";
			}
			$insert3="INSERT INTO balapathra_wyapara_short(nama,address,balapathra_ankaya,ekathuwa,masaya) VALUES('$name','$addres',$num,'$payment','$month')";
