<html>
<head><title>සතිපොළ අයකිරීම්</title>
...
<body class='body'>
	...
	<h4 align='center'>-ඇල්ල ප්‍රාදේශීය සභාව-සතිපොළ අයකිරීම්</h4>
<nav><ul>
	...
	<a href="sathipolatable.php" target="_self"  style="text-decoration:none;" target="_blank"><input  type='button' class='see' name='button' value='ගිණුම් විස්තර පිරික්සීම.'></a>

	</div>
	<div id='form1'>
	<form action='sathipola.php' method='post' >
			
			<table class='table'>
			<tr><td align='center'><b>සතිපොළ අයකිරීම් ඇතුලත් කිරීම</b></td></tr>
	...
	<?php
		...
		if(isset($_POST['enter'])){
			...
			$date="$year-$month-$day";
			$q="SELECT sum(adhi)-sum(higa) FROM `sathipola`";
			...
			if($payment>35000){
				$adhi=$payment-35000;
				$fbb=$fbb+$adhi;
				$insert3="INSERT INTO sathipola(date,masaya,masikaMudala,geviyaYuthuMudala,geweema,adhi,sub_date,sub_time) VALUES('$date','$month',35000.00,'$fbb','$payment','$adhi',CURDATE(),CURTIME())";
				$inoti="INSERT INTO prasa_two(vistharaya,geweema,Approved,wargaya) VALUES('$info','$payment','false','සතිපොළ ගාස්තු ')";
			
			}
			if($payment<35000){
				$higa=35000-$payment;
				$fbb=$fbb-$higa;
				$insert3="INSERT INTO sathipola(date,masaya,masikaMudala,geviyaYuthuMudala,geweema,higa,sub_date,sub_time) VALUES('$date','$month',35000.00,'$fbb','$payment','$higa',CURDATE(),CURTIME())";
				$inoti="INSERT INTO prasa_two(vistharaya,geweema,Approved,wargaya) VALUES('$info','$payment','false','සතිපොළ ගාස්තු ')";
			
			}
			if(mysqli_query($connection, $insert3)&& mysqli_query($connection, $inoti)){
				echo "<script>alert('සතිපොළ ගිණුම් තොරතුරු යාවත්කාලීන කරන ලදී.')</script>";
