<html>
<head><title>රාවණා ඇල්ල නැරඹුම් මැදිරිය </title>
...
<body class='body'>
	...
	<h4 align='center'>-රාවණා ඇල්ල නැරඹුම් මැදිරිය-ගාස්තු අයකිරීම</h4>
...
<nav><ul>
	...
	<a href="rawananarabumtable.php" target="_self"  style="text-decoration:none;" target="_blank"><input  type='button' class='see' name='button' value='ගිණුම් විස්තර පිරික්සීම.'></a>

	</div>
	<div id='form1' align='center'>
	<form action='rawanaella.php' method='post' >
		
			<table class='table'>
			
			<tr><td align='center'><b>රාවණා ඇල්ල-නැරඹුම් මැදිරිය ඇතුලත් කිරීම</b></td></tr>
...
<option value="31">31</option>
	...
	<?php
		...
		if(isset($_POST['enter'])){
			...
			$date="$year-$month-$day";
			$q="SELECT sum(adhi)-sum(higa) FROM rawananarabum";
			$ex2=mysqli_query($connection, $q);
			$res=mysqli_fetch_array($ex2);
			$fbb=$res[0];
			$toPay=1500.00;
			$info="දිනය: ".$date."\n මාසික ගාස්තුව:  1500.00 \n ගෙවීම: ".$payment."\nමාසය :".$month;
				
			if($payment>1500){
				$adhi=$payment-1500;
				$fbb=$fbb+$adhi;
				$insert3="INSERT INTO rawananarabum(date,masaya,masikaMudala,geviyaYuthuMudala,geweema,adhi,sub_date,sub_time) VALUES('$date','$month',1500.00,'$fbb','$payment','$adhi',CURDATE(),CURTIME())";
				$inoti="INSERT INTO prasa_two(vistharaya,geweema,Approved,wargaya) VALUES('$info','$payment','false','රාවණා ඇල්ල-නැරඹුම් මැදිරිය')";
			}
			if($payment<1500){
				$higa=1500-$payment;
				$fbb=$fbb-$higa;
				$insert3="INSERT INTO rawananarabum(date,masaya,masikaMudala,geviyaYuthuMudala,geweema,higa,sub_date,sub_time) VALUES('$date','$month',1500.00,'$fbb','$payment','$higa',CURDATE(),CURTIME())";
				$inoti="INSERT INTO prasa_two(vistharaya,geweema,Approved,wargaya) VALUES('$info','$payment','false',' රාවණා ඇල්ල-නැරඹුම් මැදිරිය')";
			}
			if(mysqli_query($connection, $insert3) && mysqli_query($connection, $inoti)){
				echo "<script>alert('රාවණා ඇල්ල-නැරඹුම් මැදිරිය ගිණුම් තොරතුරු යාවත්කාලීන කරන ලදී.')</script>";
