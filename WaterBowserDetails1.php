...
?>
	...
	<html>
		<head>
			<title align=center>  ජල බවුසරය පිලිබඳ තොරතුරු </title>
			<link rel="stylesheet" href="StylesBackhoeDetails1.css"/>
		</head>
	
		<body bgcolor="#98FFF98">
		
		<form method="post" action="WaterBowserDetails1.php" align="left">
		...
		<h2 align='center'>තොරතුරු තහවුරු කිරීම</h2>
		
		
			<form method ="post" action ="WaterBowserDetails1.php"></br>
	...
	<?php
			...
			$sel2="select * from water_bowser_owners WHERE ID='$id' order by DoP desc limit 1";
			...
			$section="ජල බවුසර් හිග මුදල ගෙවීම";
			...
			$true=1;
			$false=0;
			$insert1="update water_bowser_owners set Total=$grandtot,TotalPaid=$true  WHERE ID='$id' order by DoP desc limit 1";
			$insert2="update water_bowser_availability set Availability=$true where WNumber='W1'";
