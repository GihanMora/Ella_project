...
<body class='body'>
	...
	<h4 align="center">බලපත්‍ර ගාස්තු-ව්‍යාපාර බලපත්‍ර</h4>
	<nav><ul>
	...
	<form action='balapathra_gasthu-wyapara.php' method='post' >
			...
			<table class='table'>
				<tr><td align='center'><b>බලපත්‍ර ගාස්තු ඇතුලත් කිරීම -ව්‍යාපාර බලපත්‍ර</b></td></tr>
				<tr><td>ව්‍යාපාර බලපත්‍ර අංකය:<input type='text' required='true' name='numb' placeholder='බලපත්‍ර අංකය'></td></tr>
	...
	<div id='form'>
		<form action='balapathra_gasthu-wyapara.php' method='post'>
			<table class='table'>
				<tr><td align='center'><b>ව්‍යාපාර බලපත්‍ර ලියාපදිංචි කිරීම </b></td></tr>
	...
	<?php
		if(isset($_POST['register'])){
			...
			$insert_ppl="INSERT INTO people_info(name,address,balapathra_ankaya,wargaya) VALUES('$name1','$address1',$num1,'ව්‍යාපාර')";
			
			if(mysqli_query($connection, $insert_ppl)){
				echo "<script>alert('වයාපාර බලපත්‍ර ගාස්තු සාර්ථකව ලියාපදිංචි කරන ලදී.')</script>";
		...
		if(isset($_POST['enter'])){
			...
			$yy=date('Y');
			$ck="select nama from balapathra_wyapara WHERE balapathra_ankaya='$numb' && year='$yy'";
			...
			if($res = mysqli_fetch_array($regq)){
				if($w = mysqli_fetch_array($rr)){
					...
					$c2="select $mon from balapathra_wyapara where balapathra_ankaya='$numb' && year='$yy'";
					$ex=mysqli_query($connection, $c2);
					$res2=mysqli_fetch_array($ex);
					if($res2[0]==null){
						$insert="UPDATE balapathra_wyapara SET $mon='$payment' WHERE balapathra_ankaya='$numb' && year='$yy'";
						$insert3="INSERT INTO balapathra_wyapara_short(nama,address,balapathra_ankaya,ekathuwa,masaya,sub_date,sub_time) VALUES('$name','$addres',$numb,'$payment','$month',CURDATE(),CURTIME())";
						$inoti="INSERT INTO prasa_two(vistharaya,geweema,Approved,wargaya) VALUES('$info','$payment','false',' ව්‍යාපාර බලපත්‍ර')";
					...
					$insert="INSERT INTO balapathra_wyapara(nama,address,balapathra_ankaya,masaya,$mon,year) VALUES('$name','$addres',$numb,'$month','$payment',YEAR(CURDATE()))";
					$insert3="INSERT INTO balapathra_wyapara_short(nama,address,balapathra_ankaya,ekathuwa,masaya,sub_date,sub_time) VALUES('$name','$addres',$numb,'$payment','$month',CURDATE(),CURTIME())";
					$inoti="INSERT INTO prasa_two(vistharaya,geweema,Approved,wargaya) VALUES('$info','$payment','false',' ව්‍යාපාර බලපත්‍ර')";
				}
				if(!$invalid){
