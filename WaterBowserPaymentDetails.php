...
<?php 
	...
	$sel="select * from waterbowser_users WHERE ID = '$id_'";
	...
	$address=$row['Address'];
	$tp=$row['TP'];
	$insert="insert into waterbowserowners(Name,ID,Address,TP) values('$name','$id','$address','$tp')";
	...
	<html>
		...
		<body>
				<form  target="frame" method ="post" action ="WaterBowserPaymentDetails.php"></br>
				...
				if(isset($_POST['sdate'])){
					...
					$nod=$_POST['nod'];
					$days=(int)$nod;
					$insert="insert into waterbowseowners(DoP,DoS) values('$dop','$dos')";
