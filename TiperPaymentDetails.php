...
<?php 
	...
	$sel="select * from tiper_users WHERE ID = '$id_'";
	...
	$address=$row['Address'];
	$tp=$row['TP'];
	$insert="insert into tiper_owners(Name,ID,Address,TP) values('$name','$id','$address','$tp')";
	...
	<html>
		...
		<body>
				<form  target="frame" method ="post" action ="TiperPaymentDetails.php"></br>
				...
				if(isset($_POST['sdate'])){
					...
					$nod=$_POST['nod'];
					$days=(int)$nod;
					$insert="insert into tiper_owners(DoP,DoS) values('$dop','$dos')";
