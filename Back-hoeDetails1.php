...
?>
	...
	<html>
		<head>
			<title align=center>  බැකෝ රථය පිලිබඳ තොරතුරු </title>
			<link rel="stylesheet" href="StylesBackhoeDetails1.css"/>
		</head>
	
		<body bgcolor="#98FFF98">
		
		<form method="post" action="Back-hoeDetails1.php" align="left">
		...
		<h2 align='center'>තොරතුරු තහවුරු කිරීම හා මුදල් ගෙවීම</h2>
			...
			<form method ="post" action ="Back-hoeDetails1.php"></br>
			
		<div id="tableContainer-1">
		<div id="tableContainer-2">
		<table align="center"  height="100px" width="500px" id="myTable" >
			<tr>
	...
	<?php
			...
			$sel2="select * from back_hoe_owners WHERE ID='$id' order by DoP desc limit 1";
			$run2=mysqli_query($con,$sel2);
			$check2=mysqli_num_rows($run2);
			if($check2>0){
				
				echo '<h2 align="center">බැකෝ රථය කුලියට ගත් තොරතුරු</h2>';
			...
			$dos=$row2['DoS'];
			$dop=$row2['DoP'];
			$remainder=$row2['Remainder'];
			
			$description="".$name."  ".$id." ".$address;
			$section="බැකෝ හිග මුදල ගෙවීම";
			...
			$false=0;
						...
						$insert1="update back_hoe_owners set Total=$grandtot,TotalPaid=$true  WHERE ID='$id' order by DoP desc limit 1";
						$insert2="update back_hoe_availability set Availability=$true where BNumber='B1'";
						$insert3="insert into prasa_two(vistharaya,geweema,Approved,wargaya) values ('$description','$remainder','false','$section')";
			...
			/*$insert1="insert into back_hoe_owners(Total,TotalPaid) values('$grandtot','$true') WHERE ID='$id' order by DoP desc limit 1";
			$insert2="update back_hoe_availability set Availability=$true where BNumber='B1'";
			$insert3="insert into gamaya prasa_two(vistharaya,geweema,Approved,wargaya) values ('$description','$grandtot','false',$section) ";
			$run1=mysqli_query($con,$insert1);
