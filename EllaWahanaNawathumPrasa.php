...
<?php 
	session_start();
	$con=mysqli_connect("localhost","root","","ellawahananawathuma")or die("connection failed!");
?>
...
<html>
	...
	<?php
	if(isset($_POST['back'])){
	header("location:EllaWahanaNawathumHome.php");
	exit;
	}?>
	
			<div class='head'>
		<h1 align='center' color="black"> ඇල්ල ප්‍රාදේශීය සභාව  ඇල්ල වාහන නැවතුම්</h1>
	...
	if(isset($_POST['sub1'])){
		...
		echo '<form method ="post" action ="EllaWahanaNawathumPrasa.php" align="left">
				<input type="button" onClick="window.print() "name="print" value=" මුද්‍රණය කිරීම" />
		</form><div class = "dinaya">';
		...
		echo '<p>මෙම අවුරුද්දට අදාළ මුළු  මුදල රු.'.$advancetotal.'.00/=</br>';
			...
			echo '<form method ="post" action ="EllaWahanaNawathumPrasa.php">
